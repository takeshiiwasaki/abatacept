...
#Preparation
p=pd.read_table("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/pheno.xls")
p["ID"]=p.index
p["deltadas"]=p["das28crp w0:ch1"]-p["das28crp w16:ch1"]
p.index=range(len(p))
for i in range(len(p)):
    if (p.loc[i,"das28crp w16:ch1"] <= 3.2)&((p.loc[i,"deltadas"] >= 1.2)):    
        p.loc[i,"Response"]="Good"
    elif ((p.loc[i,"das28crp w16:ch1"]-p.loc[i,"deltadas"] > 5.1))|((p.loc[i,"deltadas"] <= 0.6)):    
        p.loc[i,"Response"]="None"
    else:
        p.loc[i,"Response"]="Moderate"

e=pd.read_table("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/expr.xls")

p["ID"]=p.index
p.index=range(len(p))
for i in range(len(p)):
    if p.loc[i,"Response"] == "Good":
        p.loc[i,"Target"]="zero"
    else:
        p.loc[i,"Target"]="one"
p=p[["ID","Target"]]
p.to_csv("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/design.xls",sep="\t",index=False)

##Gene Annotatation for expression file
a = pd.read_table("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/annot.GSE172188.xls")
a = a[["affy_hg_u133_plus_2","external_gene_name"]]
a = a.rename(columns={'affy_hg_u133_plus_2':'ID'})
e["ID"] = e.index
...
e_final.to_csv("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/expr_target_annotate.xls",sep="\t",index=True)

#Read input file
data = pd.read_table('C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/expr_target_annotate.xls')
data=data.rename(columns={'Unnamed: 0': 'ID'})
data=data.set_index("ID")
design = pd.read_table('C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/design.xls') #replace with your own design file
...
writexl.write_xlsx(r_output, "C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/limma_output.xlsx")
pd.read_excel("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/limma_output.xlsx").to_csv("C:/Users/tiwasaki/Documents/RA_RNA.ABT/GSE172188/limma_output.xls",sep="\t",index=False)
